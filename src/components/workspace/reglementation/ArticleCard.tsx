...
export const ArticleCard: React.FC<ArticleCardProps> = ({ article, onArticleClick }) => {
  return (
    <Card key={article.id} className="border-l-4 border-l-khaki-500 border-t border-r border-b border-border shadow-sm hover:shadow-md transition-all">
      <CardHeader className="pb-2">
        ...
        <CardDescription className="mt-1 text-gray-600 dark:text-gray-300">{article.description}</CardDescription>
      </CardHeader>
      <CardContent className="pt-0 pb-2">
        <p className="text-gray-600 dark:text-gray-300 text-sm">{article.content.substring(0, 200)}...</p>
        <div className="mt-2 flex flex-wrap gap-1">
          {article.keywords.map((keyword, idx) => (
            <span key={idx} className="inline-block bg-muted text-gray-600 dark:text-gray-300 text-xs px-2 py-0.5 rounded">
              #{keyword}
            </span>
          ))}
        </div>
