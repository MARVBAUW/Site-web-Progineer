        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {simulations.map((sim) => (
              <div 
                key={sim.id} 
                className={`p-3 rounded-md cursor-pointer flex justify-between items-center hover:bg-muted ${
                  currentSimulationId === sim.id ? 'bg-khaki-100 border border-khaki-200' : 'bg-card border border-border'
                }`}
          ...
          <div className="text-center py-8 text-low-contrast">
            <FileText className="h-12 w-12 mx-auto mb-2 text-gray-600 dark:text-gray-300" />
