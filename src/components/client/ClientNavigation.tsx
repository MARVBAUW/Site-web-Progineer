...
const ClientNavigation = ({ isAdminMode = false }: ClientNavigationProps) => {
  ...
  const backButton = location.pathname.includes('/admin') && isAdmin && (
    <Link
      ...
      className="flex items-center px-3 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-khaki-800 hover:bg-khaki-100 rounded-md mb-4"
    >
    ...
    <aside className="h-full flex flex-col">
      <div className="flex-1">
        <Link to="/" className="flex items-center py-4 px-4 mb-4">
          ...
          <span className="ml-1 text-sm text-low-contrast">Espace {isAdminMode && isAdmin ? 'admin' : 'client'}</span>
        </Link>
        ...
        <nav className="space-y-1 px-3">
          ...
          <div className="mb-6">
            {navItems.map(item => (
              ...
              >
                <span className="mr-3 text-low-contrast">{item.icon}</span>
                {item.label}
              </Link>
            ))}
          </div>
      ...
      <div className="p-4 border-t border-border">
        <div className="flex items-center mb-4">
          ...
          <div className="ml-3">
            <p className="text-sm font-medium text-gray-600 dark:text-gray-300 truncate w-40">
              {user?.user_metadata?.full_name || user?.email || 'Utilisateur'}
            </p>
            <p className="text-xs text-low-contrast truncate w-40">
              {user?.email}
            </p>
          </div>
        </div>
        ...
        <LogoutButton 
          variant="outline"
          size="default"
          className="w-full justify-start text-gray-600 dark:text-gray-300 hover:text-red-700 hover:bg-red-50"
        />
      </div>
    </aside>
  );
