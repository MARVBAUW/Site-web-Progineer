...
const DocumentItem: React.FC<DocumentItemProps> = ({ document, onDelete }) => {
  ...
  const getFileIcon = (type: string) => {
    switch (type) {
      ...
      default:
        return <FileText className="h-5 w-5 text-low-contrast" />;
    }
  };

  return (
    <div className="flex items-center justify-between p-3 border border-border rounded-md hover:bg-muted/50">
      <div className="flex items-center space-x-3">
        {getFileIcon(document.type)}
        <div>
          <p className="font-medium">{document.name}</p>
          <div className="flex items-center space-x-3 text-sm text-low-contrast">
      ...
      <div className="flex items-center space-x-2">
        ...
        <Button 
          variant="outline" 
          size="icon" 
          className="h-8 w-8 text-low-contrast"
