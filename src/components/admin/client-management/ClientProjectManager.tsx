...
const ClientProjectManager: React.FC<ClientProjectManagerProps> = ({ clientId }) => {
  ...
  return (
    <div className="space-y-6">
      {client && (
        <Card className="bg-muted/50 border-khaki-200">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label className="text-low-contrast text-sm">Nom</Label>
                <p className="font-medium">{client.full_name}</p>
              </div>
              <div>
                <Label className="text-low-contrast text-sm">Email</Label>
                <p className="font-medium">{client.email}</p>
              </div>
              <div>
                <Label className="text-low-contrast text-sm">Téléphone</Label>
                <p className="font-medium">{client.phone || 'Non renseigné'}</p>
              </div>
              <div>
                <Label className="text-low-contrast text-sm">Adresse</Label>
      ...
      <Card>
        ...
        <CardContent>
          {projects.length === 0 ? (
            <div className="py-8 text-center bg-muted/50 rounded-md">
              <AlertTriangle className="h-10 w-10 text-amber-500 mx-auto mb-2" />
              <h3 className="text-lg font-medium mb-1">Aucun projet trouvé</h3>
              <p className="text-low-contrast mb-4">Ce client n'a pas encore créé de projet</p>
            </div>
          ) : (
              ...
              {projects.map((project) => (
                <Card key={project.id} className="overflow-hidden">
                  <div className="flex flex-col md:flex-row">
                      ...
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                        <div>
                          ...
                          <div className="space-y-3">
                            <p className="text-gray-600 dark:text-gray-300">{project.description.slice(0, 150)}...</p>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-sm">
                              <div className="flex justify-between">
                                <span className="text-low-contrast">Type:</span>
                                <span className="font-medium">{formatConstructionType(project.construction_type)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-low-contrast">Localisation:</span>
                                <span className="font-medium">{project.location}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-low-contrast">Surface:</span>
                                <span className="font-medium">{project.surface} m²</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-low-contrast">Budget:</span>
                              ...
                              <div className="flex justify-between">
                                <span className="text-low-contrast">Statut:</span>
                                <Badge
                                  variant="outline"
                                  className={
                                    ...
                                    'bg-muted/50 text-gray-600 dark:text-gray-300 border-border'
                                  }
                                >
                            ...
                            <div className="flex flex-wrap gap-2 mt-1">
                              {project.has_pool && (
                                <Badge variant="outline" className="bg-muted/50">Piscine</Badge>
                              )}
                              {project.has_solar_panels && (
                                <Badge variant="outline" className="bg-muted/50">Panneaux solaires</Badge>
                              )}
                              {project.has_garage && (
                                <Badge variant="outline" className="bg-muted/50">Garage</Badge>
                              )}
                              {project.has_basement && (
                                <Badge variant="outline" className="bg-muted/50">Sous-sol</Badge>
                              )}
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="bg-muted/50 p-4 md:p-6 border-t md:border-t-0 md:border-l border-border flex flex-row md:flex-col justify-center md:justify-start gap-2 md:gap-3">
                      <Button 
                        variant="default" 
