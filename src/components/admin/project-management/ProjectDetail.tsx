        ...
        <CardHeader>
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
            <div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-2 text-low-contrast">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Nom du projet</span>
                    <p>{project.projectName}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Numéro de dossier</span>
                    <p>{project.fileNumber || "Non attribué"}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Type de projet</span>
                    <p>{
                ...
                <div className="space-y-3">
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Maître d'ouvrage</span>
                    <p>{project.projectOwner || "Non défini"}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Localisation</span>
                    <p>{project.location || "Non définie"}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Montant des travaux</span>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Chef de projet</span>
                    <p>{project.team?.projectManager || "Non assigné"}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Directeur technique</span>
                    <p>{project.team?.technicalDirector || "Non assigné"}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Dessinateur</span>
                    <p>{project.team?.draftsman || "Non assigné"}</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Superviseur de chantier</span>
                    <p>{project.team?.workSupervisor || "Non assigné"}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Assistant administratif</span>
                    <p>{project.team?.adminAssistant || "Non assigné"}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-low-contrast">Directeur de division</span>
            ...
            <CardContent>
              {project.companies && project.companies.length > 0 ? (
                  ...
                  {project.companies.map((company, index) => (
                    <div key={index} className="border rounded-lg p-4 bg-muted/50">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                        <div>
                          <span className="text-low-contrast">Contact : </span>
                          {company.contactName || "Non défini"}
                        </div>
                        <div>
                          <span className="text-low-contrast">Téléphone : </span>
                          {company.phone || "Non défini"}
                        </div>
                        <div>
                          <span className="text-low-contrast">Email : </span>
                          {company.email || "Non défini"}
                        </div>
                        <div>
                          <span className="text-low-contrast">Adresse : </span>
                ...
                <div className="text-center py-6">
                  <p className="text-low-contrast">Aucune entreprise n'est rattachée à ce projet pour le moment.</p>
                </div>
              )}
            </CardContent>
          </Card>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <span className="text-sm font-medium text-low-contrast">Date de début globale</span>
                      <p>{project.dates?.global?.startDate ? formatDateFrench(new Date(project.dates.global.startDate)) : "Non définie"}</p>
                    </div>
                    <div>
                      <span className="text-sm font-medium text-low-contrast">Date de fin globale</span>
