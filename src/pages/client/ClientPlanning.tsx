...
const ClientPlanning = () => {
  ...
  const getEventBadgeColor = (type: string) => {
    switch (type) {
      ...
      default:
        return "bg-muted text-gray-900 dark:text-white border-border";
    }
  };
        ...
        <div className="mt-6">
          <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700 rounded-t-lg overflow-hidden">
            {["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((day, index) => (
              <div key={index} className="bg-card p-2 text-center text-sm font-medium">
                {day}
              </div>
            ))}
          </div>
                  ...
                  <>
                    ...
                    <div className="mt-2 space-y-1">
                      {day.events?.map(event => (
                        <div 
                          key={event.id}
                          className="text-xs font-medium p-1 rounded cursor-pointer hover:bg-muted"
                        >
        ...
        <div className="space-y-4 mt-6">
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center text-sm text-low-contrast mt-1">
          ...
          {events
            ...
            .map(event => (
              <div 
                key={event.id}
                className="p-4 border border-border rounded-md hover:bg-muted/50 transition-colors opacity-80"
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-medium">{event.title}</h3>
